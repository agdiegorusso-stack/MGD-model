...
# An unresolved legacy proposition is not silently answered by the stale fact generator.
...
# Archive caps must not discard the evidence still supporting retained claims.
...
# Extend snapshots compatibly so the separate language state is also restorable.
...
# Additional guards for idle scheduling and scoped claims.
